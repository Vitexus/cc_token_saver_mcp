...
man/                  Man page source (troff format)
data/                 AppStream metainfo XML
...
Both tools are decorated with `@mcp.tool()` in `cc_token_saver_mcp/server.py`.
...
4. Update `man/cc-token-saver-mcp.1` (TOOLS EXPOSED section) and `README.md`.
